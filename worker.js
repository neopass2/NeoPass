                    ...
                    await chrome.scripting.executeScript({
                        ...
                        func: function(msg, version) {
                            ...
                            title.innerHTML = `
                                ...
                                <span style="vertical-align: middle;">NeoPass</span>
                            `;
                            title.style.cssText = `
                                display: flex;
                                align-items: center;
                            ...
                            gradientContainer.onmouseover = () => {
                                toast.style.backgroundColor = '#161616';
                            };
...
// Set up alarm when extension starts AND check immediately
chrome.runtime.onStartup.addListener(() => {
    setupUpdateAlarm();
    // Immediate update check when browser opens
    checkForUpdate();
});
...
chrome.commands.onCommand.addListener((command, tab) => {
        ...
        if (command === 'customPaste') {
            ...
            // Send message to content script to trigger batch solve
            chrome.tabs.sendMessage(tab.id, { action: 'startBatchSolve' }, (response) => {
                shortcutStates[command] = false; // Reset the state after processing
                
                if (chrome.runtime.lastError) {
                    console.log('Batch solve error:', chrome.runtime.lastError.message);
                } else if (response && response.success) {
                    console.log('Batch solve completed:', response.message);
                } else if (response && response.error) {
                    console.log('Batch solve failed:', response.error);
                }
            });
        }
});
    ...
    if (message.action === "getMcqCredits") {
        (async () => {
            try {
                ...
                if (!accessToken || !refreshToken) {
                    sendResponse({ success: false, error: 'Not logged in', mcqCredits: 0 });
                    return;
                }
                ...
                if (!response.ok && (response.status === 401 || response.status === 403)) {
                    const newAccessToken = await refreshAccessToken(refreshToken);
                    if (newAccessToken === 'network_error') {
                        sendResponse({ success: false, error: 'Network error', mcqCredits: 0 });
                        return;
                    }
                    if (!newAccessToken) {
                        sendResponse({ success: false, error: 'Session expired', mcqCredits: 0 });
                        return;
                    }
                ...
                if (!response.ok) {
                    sendResponse({ success: false, error: 'Failed to get account info', mcqCredits: 0 });
                    return;
                }
                
                const data = await response.json();
                // Account data is nested under data.account
                const mcqCredits = data.account?.mcqCredits !== undefined ? data.account.mcqCredits : 0;
                
                console.log(`[getMcqCredits] User has ${mcqCredits} MCQ credits`);
                sendResponse({ success: true, mcqCredits: mcqCredits });
                
            } catch (error) {
                console.error('[getMcqCredits] Error:', error);
                sendResponse({ success: false, error: error.message, mcqCredits: 0 });
            }
        })();
    ...
    if (message.action === "batchSolve") {
        (async () => {
            try {
                ...
                if (!accessToken || !refreshToken) {
                    throw new Error('Please login to use batch solve. Click the extension icon to login.');
                }
                ...
                let response = await makeAuthenticatedRequest(API_URL, 'POST', accessToken, { prompt, questionCount });
                
                // Handle token expiration
                if (!response.ok && (response.status === 401 || response.status === 403)) {
                    console.log('[Batch Solve] Token expired, attempting refresh...');
                    const newAccessToken = await refreshAccessToken(refreshToken);
                    ...
                    response = await makeAuthenticatedRequest(API_URL, 'POST', newAccessToken, { prompt, questionCount });
                }
...
async function handleChatMessage(message, sender) {
    try {
        // Check if user is logged in (REQUIRED for chat)
        const {
            accessToken,
            refreshToken
        } = await getTokens();
        ...
        if (!accessToken || !refreshToken) {
            sendChatErrorResponse(sender.tab.id, "Please login to use chat features. Click the extension icon to login.");
            return;
        }
        ...
        if (!response.ok && (response.status === 401 || response.status === 403)) {
            ...
            if (newAccessToken === 'network_error') {
                ...
                sendChatErrorResponse(sender.tab.id, "Network error. Please check your connection and try again.");
                return;
            }

            if (!newAccessToken) {
                // Clear stored tokens - refresh token is invalid
                chrome.storage.local.remove(['accessToken', 'refreshToken', 'loggedIn']);
...
// Add this at the end of worker.js
